...
mod test {
    ...
    /// the raw string would therefore send consumers to a different machine than refbox calls, so
    /// the normalised form is reported instead.
    #[test]
    fn a_backslash_address_is_reported_in_its_normalised_form() {
        assert_eq!(
