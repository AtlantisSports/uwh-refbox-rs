...
**Further walkthrough findings (2026-04-23):**

1. **Banner-height regression from Task 12.** The health tile was introduced at 130x130 px, which expanded the time banner's height by 41 pixels compared to the pre-feature layout. Flagged during the walkthrough; fixed in commits `9660aed` (short-banner sizing) and `fa50a6b` (tall-banner Main page scaling). `HEALTH_TILE_SIZE` now equals `MIN_BUTTON_SIZE` and the tile scales to match banner height on tall pages, keeping the status-dot ratio consistent.

2. **`post_game_stats` rejected as 400 against under-configured events.** Portal's `AdminController.EventStats.PushGameEvents` handler requires `CapNumberConstraints != None` AND `CapNumber == Required` on the event; an under-configured test event 400s every stats submission (even empty payloads) before looking at the body. Unblocked the walkthrough by reconfiguring the dev test event's cap-number constraints to `Unique1_99`. Design question about refbox-side behaviour (skip-when-empty / tolerate-400 / parse-reason) captured in proposed ADR 015.

3. **10-second green-checkmark overlay removed.** After directly witnessing the overlay once (on Scenario 1 Game 3 end), operator review concluded it is redundant with the detail-page "Submitted N min ago" rows accessible on tile-tap. Shipped in commit `5038231`. ADR 011 amended with a superseding subsection; `check_circle.svg` asset deleted; two unit tests removed.

4. **Latent `last_attempt_at` / `attempts` stamping issue surfaced but not fixed.** Queued items show "0 attempts" and "tap to retry" on the detail page regardless of how many retries the background loop has actually performed, because `attempt_item` in `portal_manager/health.rs` never stamps those fields. Task 15 code review flagged this as a carry-forward concern. Not blocking the walkthrough or the PR — the queue behaves correctly, only the labels mislead. Candidate for a separate small fix branch after the portal health PR merges.
