> **SUPERSEDED on 2026-09-11.** The stay-blank rule described below — discounting the slot's
> spare time (`Game::game_block_buffer()`) from the in-game figure — no longer exists. DELAY is
> now measured against the next game's **published start time** and does not use the Game Block
> at all: a game must end by `next scheduled start - minimum_break`, and DELAY is how far past
> that point it is projected to run. On the last game of a court, where there is no next game,
> it falls back to how late that game is running against its own scheduled start.
>
> Both "Decisions (locked with the user)" and all five acceptance criteria below are retired —
> in particular criterion 3, "a game that starts late but whose slot can recover it shows blank",
> is no longer the behaviour. Superseded by the commit
> `fix(refbox): show delay against the next scheduled start`.
>
> This document was never committed at the time; it is added here, after the fact, because the
> code cited it by name and the reasoning for the behaviour it describes would otherwise be lost.
