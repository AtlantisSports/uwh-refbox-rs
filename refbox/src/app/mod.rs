...
impl RefBoxApp {
    ...
    /// task sees it on its next tick. This is the only place that
    /// writes `current_event_id` after construction; keeping the two
    /// fields in sync here means the tile's `verify_token` leg reflects
    /// the operator's actual event selection.
    ...
    fn apply_settings_change(&mut self) {
        ...
        self.set_current_event_id(current_event_id);
    ...
    pub(super) fn update(&mut self, message: Message) -> Task<Message> {
        ...
        match message {
            ...
            Message::ParameterEditComplete { canceled } => {
                ...
                // so we route back to the detail page directly. The
                // RecvPortalToken handler will replace this once the network
                // request completes.
                let next_state = match self.app_state {
                    ...
                    AppState::ParameterList(param, _) => match param {
                        ListableParameter::Game => AppState::EditGameConfig(ConfigPage::Main),
