...
//! outage would be permanent for the rest of the run. It fires on a fixed interval
//! ([`PORTAL_REFRESH_INTERVAL`]) and also whenever [`consume_snapshots`] notices the event id or
//! game number has changed, so a new game's teams do not wait out a full idle interval to appear.
//!
//! # Which game is "next"
//!
...
/// public API (see `portal`'s module doc).
...
/// forever, retrying on every tick (or eager wake) regardless of whether the previous attempt
/// succeeded: a failed fetch is silently absorbed by [`Directory`]'s own never-fatal contract, and
/// this loop simply tries again next time, never giving up for the rest of the run.
pub async fn refresh_portal_loop(state: Arc<AppState>, refresh_notify: Arc<Notify>) {
    let mut ticker = interval(PORTAL_REFRESH_INTERVAL);
...
mod tests {
    ...
    #[tokio::test]
    async fn portal_refresh_loop_retries_after_failure_without_disturbing_feed_data() {
        // Deliberately real time, not `start_paused`: each refresh cycle here is a genuine
        // two-task TCP round trip (this test's own fake portal server has to actually accept,
        // read and respond on a separate spawned task), and tokio's paused-clock auto-advance is
        // unreliable across that kind of real socket I/O -- it can fast-forward the virtual clock
        // past a timer before the real OS-level I/O it was racing against has actually completed,
        // which was observed to make even a single request spuriously time out. `feed.rs`'s own
        // paused-clock tests avoid this because a refused connection fails immediately at the OS
        // level, with no real request/response exchange to race against.
        let listener = TcpListener::bind("127.0.0.1:0")
            .await
            .expect("bind a local listener");
        let portal_addr = listener.local_addr().expect("local_addr");
        ...
        *write_lock(&state.directory) = Some(Arc::new(Directory::new(
        ...
        // Drive several refresh cycles through the same eager-wake path a real event/game-number
        // change uses (see `consume_snapshots`), rather than waiting out real 15-second ticks --
        // proves the loop's retry behaviour without a slow test. A short real sleep after each
        // notification gives that cycle's real network round trip time to finish before the next
        // one is requested.
        for _ in 0..4 {
            refresh_notify.notify_one();
        ...
        assert!(
            seen >= 3,
            "the refresh loop should have retried after each failure rather than giving up \
             after the first one, got {seen} attempt(s)"
        );

        // The failing Portal must never have touched the game data that came from the refbox.
        let display = current_display(&state);
        assert_eq!(
            display.snapshot.scores.black, 5,
            "a Portal refresh failure must never disturb game data from the refbox"
        );
