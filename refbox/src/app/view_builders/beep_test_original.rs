...
use iced::{
    ...
    widget::{Space, button, column, container, row, text, Button, Column, Container, Row, Text, horizontal_space},
};
...
impl Level {
    pub fn migrate(old: &toml::Table) -> Self {
        let Self {
            mut count,
            mut duration,
        } = Default::default();
...
impl BeepTestState {
    ...
    fn advance_period(&mut self) {
        let current_period = self.snapshot.current_period.clone();
...
// Copy exact macros from original
macro_rules! column {
    () => (
...
pub fn make_button<'a, Message: Clone>(
...
// Helper function for time formatting matching original
fn secs_to_long_time_string(secs: u32) -> String {
    let min = secs / 60;
    let sec = secs % 60;
    format!("{min:5}:{sec:02}")
}
