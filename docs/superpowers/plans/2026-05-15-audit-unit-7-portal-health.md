...
**1. Task 6 Fix 2 scope expanded from 2 files to 3 files during execution.** Plan-specified scope was `refbox/src/app/view_builders/portal_detail.rs` + `refbox/translations/en-US/refbox.ftl`. The implementer subagent returned `BLOCKED` after discovering that the audit-window commit `5de76ed` had stripped the `attempts` field from `DetailRow::Pending` along with the suffix rendering. The principal authorized adding `refbox/src/portal_manager/mod.rs` as a 3rd file. Operator-approved during execution. Surgical fix landed as commit `38482fd`. Also discovered (and acted on): the pre-strip `stats_only` field was always hardcoded `false` — dead code — so the operator's literal Task 5 choice "restore only the attempt count" was honored exactly (not "attempt count + stats-only" which I had erroneously included in some framing).

**2. Task 7 walkthrough environment differed from spec § Setup.** Spec § 5 prescribed local uwh-portal at `localhost:5000`; operator chose to skip local setup and try dev portal as fallback. First dev-portal attempt against `UWH_PORTAL_URL_OVERRIDE=https://dev.uwhportal.com` failed because that hostname is the web frontend, not the API. Discovered the correct dev API URL `https://api.dev.uwhportal.com` by reading `refbox/src/config.rs:71`'s production default (`https://api.uwhportal.com` — separate `api.` subdomain) and applying the convention. Second walkthrough against `https://api.dev.uwhportal.com` verified 7 of 8 scenarios. Recorded as Findings backlog #7 + memory `reference_dev_portal_url` + Process refinement #3.

**3. Task 7 Scenario 2 verified at 1-minute STUCK_THRESHOLD substitute instead of 30 minutes.** Plan anticipated this as the operator's workaround for the 30-minute wallclock wait (Step 7.7 already described it). Executed: working-tree-only edit reducing `STUCK_THRESHOLD` from `TimeDuration::minutes(30)` → `TimeDuration::minutes(1)`; full green → yellow → red → green chain verified via WiFi toggle; threshold reverted to 30 minutes after walkthrough; `git diff` clean post-revert. The 30-minute production value is unit-tested via `refbox/src/portal_manager/health.rs`. Recorded in ADR 011 Verified-by section's "What was not verified" + Process refinement #4.

**4. Task 7 walkthrough completeness was 7-of-8, not 8-of-8.** Plan envisioned all 8 Gherkin scenarios `@tested_pass`; actual was 7 `@tested_pass` + 1 `@tested_deferred` (Scenario 3 — induced 409 conflict — skipped on operator decision). The deferred scenario's mechanics are exercised indirectly by Scenario 4 (two-tap discard) and the FORCE-button code path. Recorded in ADR 011 Verified-by section.

**5. Task 3 catalog construction took the parallel-dispatch path.** Plan offered both parallel and sequential options for the 5 group subagents. Principal chose parallel (single message with 5 Agent calls). Wall clock ~8 minutes for all five vs estimated ~25 minutes sequential. Two subagents flagged grouping-doc misattributions that the principal would have missed working alone (`3277a47` doesn't touch `keypad_pages/mod.rs`; `b122f1d` touches `main_view.rs` not `game_info.rs`). Recorded as Process refinement #1.

**6. First walkthrough attempt was prematurely abandoned, then resumed.** Initial framing after the dev-URL-hits-web-frontend issue was to defer 7 of 8 scenarios and move to Task 8. Operator correctly pushed back ("don't we need to run through the program again for the tests we couldn't verify?"). Process lesson recorded as memory `feedback_explore_env_before_deferring`. The walkthrough notes commit (`55f3165`) records the first attempt; the corrected-URL commit (`cfca9ab`) records the actual 7-of-8 verification. Two walkthrough-notes commits rather than the planned one as a result.
