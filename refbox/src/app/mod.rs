...
impl RefBoxApp {
    ...
    fn apply_app_options(&mut self) -> Option<ConfirmationKind> {
        ...
        // No manual-switch handling here, deliberately. This page has no source
        // control — its only cycle is App Mode (Hockey/Rugby/BeepTest) — and the
        // MANUAL GAMES button lives on the Game page, where the change is staged
        // and applied. A staged source cannot be carried here either: the Game
        // page's only exits are its own APPLY and a Cancel whose
        // `revert_from_snapshot` restores `source` from `PageEntrySnapshot::Game`.
        // So `edited.source` always equals the committed source by the time this
        // runs, and a guard on "remote now, Manual staged" asks for both at once.
        //
        // This branch previously carried ~30 lines doing exactly that, including a
        // clock restart. Removed 2026-09-04 after findings 3 and 4 of the review
        // were traced to a path the UI has never offered — not on master, and not
        // on this branch's own original base. Switching to manual is handled where
        // it can actually happen: `apply_game_options`, which raises
        // `SwitchToManualFromApply` mid-game and resets via `reset_to_manual_break`.
        ...
        let mut main_view = column![match self.app_state {
            AppState::MainPage => {
                ...
                let behind_schedule = if self.config.show_behind_schedule_time {
                    self.tm.lock().behind_schedule(Instant::now())
                } else {
